...
int main (int argc, char *argv[])
{
	...
	do
	{
		...
		for (conn_t* cur_conn = pool->conn_head;cur_conn != NULL;/* check all descriptors, stop when checked all valid fds */)
		{
			...
			/*******************************************************/
			if (check != 1 && FD_ISSET(cur_conn->fd,&pool->ready_write_set)!= 0) {
